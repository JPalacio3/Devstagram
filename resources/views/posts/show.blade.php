        ...
        @if ($post->user_id === auth()->user()->id)
          <form action="{{ route('posts.destroy', $post) }}" method="POST">
            ...
            <input type="submit" value="Eliminar Publicación"
              class="bg-red-500 hover:bg-red-800 p-2 rounded text-white font-bold mt-4 cursor-pointer" />
          </form>
        @endif
      @endauth
    </div>
    ...
    <div class="md:w-1/2 p-5">
      ...
      <div class="shadow bg-white p-5 mb-5 rounded-xl">
        ...
        @auth
          ...
          @if (session('mensaje'))
            <div
              class="bg-green-600 p-2 rounded-lg mb-6 text-white text-center uppercase font-bold">
              {{ session('mensaje') }}
            </div>
          @endif
          ...
          <form
            ...
            <div class="mb-5 ">
              ...
              @error('comentario')
                <p
                  class = "bg-red-500 text-white my-2 rounded-xl text-sm p-2 text-center font-bold">
                  {{ $message }}
                </p>
              @enderror
            </div>
